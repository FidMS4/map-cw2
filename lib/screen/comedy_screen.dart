          ...
          children: [
            FlipPoster(
              front: Container(
                height: posterHeight,
                width: posterWidth,
                child: Image.network(comedyList[index].imageURL),
              ),
              back: Container(
                ...
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                      comedyListBackside[index].gif,
                      Padding(
                        padding: const EdgeInsets.all(5.0),
                        child: comedyListBackside[index].summary,
                      ),
                    ],
                  ),
                ),
            ...
            Padding(
              padding: const EdgeInsets.all(5.0),
              child: comedyList[index].title,
            ),
            Row(
              children: [
                for (var i = 0; i < comedyList[index].stars; i++)
                  Icon(Icons.star, color: Colors.yellow[800]),
                for (var i = 0; i < comedyList[index].halfStar; i++)
  ...
  void addFavorites() {
    state.render(() {
      for (int i = 0; i <= selected.length - 1; i++) {
        favoritesList.insert(selected[i], comedyList.elementAt(selected[i]));
      }
      selected = null;
    });
  }
